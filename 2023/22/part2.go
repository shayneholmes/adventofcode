...
func main() {
	...
	sum := 0
	for bi := range bricks {
		// how many bricks would fall if we disintegrated this one?
		fallen := map[int]bool{bi: true}
		// BFS over supports
		q := []int{bi}
		for len(q) > 0 {
			b := q[0]
			...
			if !issupported || fallen[b] {
				fallen[b] = true
				q = append(q, supportedby[b]...)
			}
		}
		fmt.Printf("remove %d and %v would fall\n", bi, fallen)
		sum += len(fallen) - 1
	}
	fmt.Println(sum)
